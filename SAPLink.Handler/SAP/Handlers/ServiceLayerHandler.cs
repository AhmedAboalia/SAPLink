...
public partial class ServiceLayerHandler : IServiceLayerHandler
{
    ...
    public void Connect()
    {
        var response = SAPHttpClientFactory.Initialize("Login", Method.POST, _client);
    ...
    {
        var response = SAPHttpClientFactory.Initialize("Login", Method.POST, _client,  loginTypes, loginData);
        if (response.StatusCode == HttpStatusCode.OK)
        {
            _session = JsonConvert.DeserializeObject<Session>(response.Content);
            IsConnected = true;
    ...
    {
        var response = SAPHttpClientFactory.Initialize("Logout", Method.POST, _client);
    ...
    public List<PriceList> GetAllPriceLists()
    {
        ...
        try
        {
            ...
            response = SAPHttpClientFactory.Initialize(query, Method.GET, _client, LoginModel.LoginTypes.Basic, null, body);
