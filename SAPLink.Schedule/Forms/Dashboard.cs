...
public partial class Dashboard : Form
{
    ...
    private List<Schedules> GetSchedules()
    {
        ...
        //var allowedDocuments = new[] { 3, 4, 7,8,10 };

        return _unitOfWork.Schedule.GetAll(includes).ToList();
        //return _unitOfWork.Schedule.GetAll(includes).Where(x=> allowedDocuments.Contains(x.Id)).ToList();
    }
    ...
    private void ComboBoxDocuments_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        var scheduledDocument = GetSchedule(selectedIndex);

        if (scheduledDocument.Times.Any())
        {
            foreach (var times in scheduledDocument.Times)
            {
                if (times.TimeId == 1)
                {
                    textBoxHour1.Text = times.Time.ToString("hh:mm tt");
                    checkBox1.Checked = times.Active;
                }


                if (times.TimeId == 2)
                {
                    textBoxHour2.Text = times.Time.ToString("hh:mm tt");
                    checkBox2.Checked = times.Active;
                }

                if (times.TimeId == 3)
                {
                    textBoxHour3.Text = times.Time.ToString("hh:mm tt");
                    checkBox3.Checked = times.Active;
                }
            }
        }
        else
    ...
    private void buttonSaveToDatabase_Click(object sender, EventArgs e)
    {
        if (buttonScheduleIt.Enabled)
        {
            ...
            try
            {
                ...
                if (schedule != null 
                    && checkBox1.Checked && textBoxHour1.Text.IsHasValue()
                    && checkBox2.Checked && textBoxHour2.Text.IsHasValue()
                    && checkBox3.Checked && textBoxHour3.Text.IsHasValue()
                    //&& checkBox4.Checked && textBoxHour4.Text.IsHasValue()
                    )
                {
